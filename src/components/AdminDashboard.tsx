...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout }) => {
  ...
  const filteredSamples = samples.filter(sample => {
    // 1. Filtro por Tipo
    const matchType = filterType === 'ALL' || sample.type === filterType;
    
    // 2. Filtro por Texto (Cidade, Bairro, Endereço ou Subtipo)
    const searchLower = searchTerm.toLowerCase();
    const matchText = 
      sample.city.toLowerCase().includes(searchLower) ||
      (sample.neighborhood || '').toLowerCase().includes(searchLower) ||
      (sample.address || '').toLowerCase().includes(searchLower) ||
      (sample.urbanSubType || '').toLowerCase().includes(searchLower) ||
      (sample.ruralActivity || '').toLowerCase().includes(searchLower);
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
        ...
        {activeTab === 'LIST' ? (
          <div className="animate-fade-in">
            {/* --- BARRA DE FILTROS --- */}
            <div className="bg-gray-50 p-4 rounded-lg mb-6 flex flex-col md:flex-row gap-4 items-center border border-gray-200">
              <div className="w-full md:w-auto">
                <label className="block text-xs font-bold text-gray-500 mb-1">Filtrar por Tipo</label>
                <select 
                  className="w-full md:w-40 border border-gray-300 rounded-lg p-2 text-sm focus:ring-2 focus:ring-agro-500 outline-none"
                  value={filterType}
                  onChange={(e) => setFilterType(e.target.value as any)}
                >
                  <option value="ALL">Todos</option>
                  <option value={PropertyType.URBAN}>Urbanos</option>
                  <option value={PropertyType.RURAL}>Rurais</option>
                </select>
              </div>
              
              <div className="w-full">
                <label className="block text-xs font-bold text-gray-500 mb-1">Buscar (Cidade, Bairro, Endereço...)</label>
                <div className="relative">
                  <input 
                    type="text" 
                    className="w-full border border-gray-300 rounded-lg p-2 pl-9 text-sm focus:ring-2 focus:ring-agro-500 outline-none"
              ...
              <div className="w-full md:w-auto text-right md:text-left mt-2 md:mt-0">
                <span className="text-xs font-bold text-gray-500 bg-white px-3 py-1 rounded-full border border-gray-200">
                  {filteredSamples.length} amostras
                </span>
              </div>
            </div>
              ...
              <table className="w-full text-sm text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100">
                  ...
                  {filteredSamples.length === 0 && (
                    <tr><td colSpan={5} className="p-8 text-center text-gray-400 italic">Nenhuma amostra encontrada com estes filtros.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
